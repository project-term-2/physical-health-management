const carbo_flourList=firebase.firestore();
const table=document.querySelector('#addCarbo_flour');

db.collection('carbo_flour').get().then(carbo_flour => {
    carbo_flour.docs.forEach(doc => {
        renderCarbo_flour(doc);
    
    });
});

function renderCarbo_flour(doc) {
   ...
   /*  Array.from(document.getElementsByClassName('data-id',doc.id)).forEach((button) => {
    ...
    btn.addEventListener('click',(e)=>{
        ...
        db.collection('carbo_flour').doc(id).addEventListener();
    });
}
