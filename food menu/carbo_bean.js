const carbo_beanList=firebase.firestore();
const table=document.querySelector('#addCarbo_bean');

db.collection('carbo_bean').get().then(carbo_bean => {
    carbo_bean.docs.forEach(doc => {
        renderCarbo_bean(doc);
    
    });
});

function renderCarbo_bean(doc) {
   ...
   /*  Array.from(document.getElementsByClassName('data-id',doc.id)).forEach((button) => {
    ...
    btn.addEventListener('click',(e)=>{
        ...
        db.collection('carbo_bean').doc(id).addEventListener();
    });
}
