const noodleList=firebase.firestore();
const table=document.querySelector('#addNoodle');

db.collection('noodle').get().then(noodle => {
    noodle.docs.forEach(doc => {
        renderNoodle(doc);
    
    });
});

function renderNoodle(doc) {
   ...
   /*  Array.from(document.getElementsByClassName('data-id',doc.id)).forEach((button) => {
    ...
    btn.addEventListener('click',(e) => {
        ...
        db.collection('noodle').doc(id).add();
    });
}
