...
const fat_vegetableList=firebase.firestore();
const table=document.querySelector('#addFat_vegetable');

db.collection('fat_vegetable').get().then(fat_vegetable => {
    fat_vegetable.docs.forEach(doc => {
        renderFat_vegetable(doc);
    
    });
});

function renderFat_vegetable(doc) {
   ...
   /*  Array.from(document.getElementsByClassName('data-id',doc.id)).forEach((button) => {
    ...
    btn.addEventListener('click',(e)=>{
        ...
        db.collection('fat_vegetable').doc(id).addEventListener();
    });
}
