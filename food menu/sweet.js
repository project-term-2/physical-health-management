...
const fruitList=firebase.firestore();
const table=document.querySelector('#addSweet');

db.collection('sweet').get().then(sweet => {
    sweet.docs.forEach(doc => {
        renderSweet(doc);
    
    });
});

function renderSweet(doc) {
   ...
   /*  Array.from(document.getElementsByClassName('data-id',doc.id)).forEach((button) => {
    ...
    btn.addEventListener('click',(e)=>{
        ...
        db.collection('sweet').doc(id).addEventListener();
    });
}
