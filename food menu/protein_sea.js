...
const protein_seaList=firebase.firestore();
const table=document.querySelector('#addProtein_sea');

db.collection('protein_sea').get().then(protein_sea => {
    protein_sea.docs.forEach(doc => {
        renderProtein_sea(doc);
    
    });
});

function renderProtein_sea(doc) {
   ...
   /*  Array.from(document.getElementsByClassName('data-id',doc.id)).forEach((button) => {
    ...
    btn.addEventListener('click',(e)=>{
        ...
        db.collection('protein_sea').doc(id).addEventListener();
    });
}
