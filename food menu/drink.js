const drinkList=firebase.firestore();
const table=document.querySelector('#addDrink');

db.collection('drink').get().then(drink => {
    drink.docs.forEach(doc => {
        renderDrink(doc);
    
    });
});

function renderDrink(doc) {
   ...
   /*  Array.from(document.getElementsByClassName('data-id',doc.id)).forEach((button) => {
    ...
    btn.addEventListener('click',(e)=>{
        ...
        db.collection('drink').doc(id).addEventListener();
    });
}
