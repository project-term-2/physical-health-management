const sidedishList=firebase.firestore();
const table=document.querySelector('#addSidedish');

db.collection('sidedish').get().then(sidedish => {
    sidedish.docs.forEach(doc => {
        renderSidedish(doc);
    
    });
});

function renderSidedish(doc) {
   ...
   /*  Array.from(document.getElementsByClassName('data-id',doc.id)).forEach((button) => {
    ...
    btn.addEventListener('click',(e) => {
        ...
        db.collection('sidedish').doc(id).add();
    });
}
