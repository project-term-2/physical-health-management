...
const protein_eggList=firebase.firestore();
const table=document.querySelector('#addProtein_egg');

db.collection('protein_egg').get().then(protein_egg => {
    protein_egg.docs.forEach(doc => {
        renderProtein_egg(doc);
    
    });
});

function renderProtein_egg(doc) {
   ...
   /*  Array.from(document.getElementsByClassName('data-id',doc.id)).forEach((button) => {
    ...
    btn.addEventListener('click',(e)=>{
        ...
        db.collection('protein_egg').doc(id).addEventListener();
    });
}
