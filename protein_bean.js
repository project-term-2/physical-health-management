...
const protein_beanList=firebase.firestore();
const table=document.querySelector('#addProtein_bean');

db.collection('protein_bean').get().then(protein_bean => {
    protein_bean.docs.forEach(doc => {
        renderProtein_bean(doc);
    
    });
});

function renderProtein_bean(doc) {
   ...
   /*  Array.from(document.getElementsByClassName('data-id',doc.id)).forEach((button) => {
    ...
    btn.addEventListener('click',(e)=>{
        ...
        db.collection('protein_bean').doc(id).addEventListener();
    });
}
