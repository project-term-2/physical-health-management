const soupList=firebase.firestore();
const table=document.querySelector('#addSoup');

db.collection('soup').get().then(soup => {
    soup.docs.forEach(doc => {
        renderSoup(doc);
    
    });
});

function renderSoup(doc) {
   ...
   /*  Array.from(document.getElementsByClassName('data-id',doc.id)).forEach((button) => {
    ...
    btn.addEventListener('click',(e)=>{
        ...
        db.collection('soup').doc(id).addEventListener();
    });
}
