const fat_meatList=firebase.firestore();
const table=document.querySelector('#addFat_meat');

db.collection('fat_meat').get().then(fat_meat => {
    fat_meat.docs.forEach(doc => {
        renderFat_meat(doc);
    
    });
});

function renderFat_meat(doc) {
   ...
   /*  Array.from(document.getElementsByClassName('data-id',doc.id)).forEach((button) => {
    ...
    btn.addEventListener('click',(e)=>{
        ...
        db.collection('fat_meat').doc(id).addEventListener();
    });
}
