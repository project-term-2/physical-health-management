const singledishList=firebase.firestore();
const table=document.querySelector('#addSingledish');

db.collection('singledish').get().then(singledish => {
    singledish.docs.forEach(doc => {
        renderSingledish(doc);
    
    });
});

function renderSingledish(doc) {
   ...
   /*  Array.from(document.getElementsByClassName('data-id',doc.id)).forEach((button) => {
    ...
    btn.addEventListener('click',(e) => {
        ...
        db.collection('singledish').doc(id).add();
    });
}
