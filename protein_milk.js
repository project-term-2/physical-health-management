...
const protein_milkList=firebase.firestore();
const table=document.querySelector('#addProtein_milk');

db.collection('protein_milk').get().then(protein_milk => {
    protein_milk.docs.forEach(doc => {
        renderProtein_milk(doc);
    
    });
});

function renderProtein_milk(doc) {
   ...
   /*  Array.from(document.getElementsByClassName('data-id',doc.id)).forEach((button) => {
    ...
    btn.addEventListener('click',(e)=>{
        ...
        db.collection('protein_milk').doc(id).addEventListener();
    });
}
